...
export const checkNewEvents = action({
  ...
  handler: async (ctx) => {
    ...
    // Use internal query to access raw settings (including API keys)
    const settings = await ctx.runQuery(
      internal.notifications.getSettingsInternal,
    );
    ...
    if (settings.notifyOnNewOrder !== false) {
      const newOrders = await ctx.runQuery(api.orders.getRecentOrders, {
    ...
    if (settings.notifyOnLowStock !== false) {
      ...
      for (const item of lowStockItems) {
        ...
        const recentNotifications = await ctx.runQuery(
          api.notifications.getRecentByType,
          {
            type: "low_stock",
    ...
    if (settings.notifyOnNewCustomer !== false) {
      const newCustomers = await ctx.runQuery(api.users.getRecentUsers, {
        since: fiveMinutesAgo,
      });
